    ...
    AuditService audit) : ControllerBase
{
    ...
    {
        var companyError = await ValidateSelectedCompany(request.CompanyId);
        ...
        UpdateCareHomeRequest request)
    {
        ...
        var companyError = await ValidateSelectedCompany(request.CompanyId, careHome.CompanyId);
        ...
        careHome.CompanyId = NormalizeCompanyId(request.CompanyId);
        careHome.Code = code;
        ...
        careHome.BankDetails = NormalizeText(request.BankDetails);
    ...
    public async Task<IActionResult> GetLogo(string key)
    {
        ...
        if (careHome is null
            || string.IsNullOrWhiteSpace(careHome.LogoPath)
            || !await userAccess.CanAccessCareHomeAsync(tenantContext.TenantId, careHome.Id))
        {
            return NotFound();
        }

        var bytes = await documents.ReadAsync(careHome.LogoPath);
        return bytes is null
            ? NotFound()
            : File(bytes, LogoStorage.ContentType(careHome.LogoPath));
    }

    [HttpPost("{key}/logo")]
    [RequestSizeLimit(LogoStorage.MaxBytes)]
    public async Task<ActionResult<CareHomeDto>> UploadLogo(string key, IFormFile file)
    {
        ...
        var careHome = await dbContext.CareHomes.FirstOrDefaultAsync(x =>
            x.TenantId == tenantContext.TenantId &&
            (publicId != default ? x.PublicId == publicId : x.Id == id));

        if (careHome is null || !await userAccess.CanAccessCareHomeAsync(tenantContext.TenantId, careHome.Id))
        {
            return NotFound();
        }
        ...
        var tenantPublicId = tenantContext.TenantPublicId is Guid known && known != Guid.Empty
            ? known
            : await dbContext.Tenants.Where(x => x.Id == tenantContext.TenantId).Select(x => x.PublicId).FirstAsync();
        careHome.LogoPath = await documents.SaveAsync(
            TenantDocumentPaths.Folder(tenantPublicId, "logos"),
            $"care-home-{careHome.PublicId:N}{extension}",
            buffer.ToArray());
        await dbContext.SaveChangesAsync();
        await audit.LogAsync("CareHome", careHome.Id.ToString(), "UploadLogo", null, new { careHome.LogoPath }, "Uploaded care home logo.");

        var dto = await ProjectToDto(dbContext.CareHomes.AsNoTracking())
            .FirstAsync(x => x.Id == careHome.Id);
        return Ok(dto);
    }
