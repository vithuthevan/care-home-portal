    ...
    ClientIdentifierService clientIdentifiers) : ControllerBase
{
        ...
        if (!string.IsNullOrWhiteSpace(company))
        {
            ...
            query = query.Where(x =>
                companyPublicId != default
                    ? x.CareHome.Company != null && x.CareHome.Company.PublicId == companyPublicId
        ...
        CreateClientRequest request)
    {
        ...
        await ApplyGuardianAsync(client, request.GuardianName, request.GuardianRelationship, request.GuardianEmail, request.GuardianPhone, request.GuardianAddress);
        ...
        IQueryable<Client> query)
    {
        return query.Select(x => new ClientDto
        {
            ...
            CompanyPublicId = x.CareHome.Company != null ? x.CareHome.Company.PublicId : null,
            CareHomeName = x.CareHome.Name,
            CompanyName = x.CareHome.Company != null ? x.CareHome.Company.Name : string.Empty,
            SageId = x.SageId,
            ...
            PrimaryFundingAuthorityName = x.FundingContracts
                .Where(c => c.Status == "Active")
                .OrderBy(c => c.Id)
                .Select(c => c.FundingAuthority.Name)
                .FirstOrDefault(),
            GuardianName = x.Guardian != null ? x.Guardian.Name : null,
            GuardianRelationship = x.Guardian != null ? x.Guardian.Relationship : null,
            GuardianEmail = x.Guardian != null ? x.Guardian.Email : null,
            GuardianPhone = x.Guardian != null ? x.Guardian.Phone : null,
            GuardianAddress = x.Guardian != null ? x.Guardian.Address : null
        });
    }

    private async Task ApplyGuardianAsync(
        Client client,
        string? name,
        string? relationship,
        string? email,
        ...
        string? address)
    {
        var showGuardian = await dbContext.TenantSettings.AsNoTracking()
            .Where(x => x.TenantId == tenantContext.TenantId)
            .Select(x => x.ShowGuardian)
            .FirstOrDefaultAsync();
        if (!showGuardian)
        {
            return;
        ...
        var guardian = await dbContext.ClientGuardians
            .FirstOrDefaultAsync(x => x.ClientId == client.Id && x.TenantId == tenantContext.TenantId);

        if (string.IsNullOrWhiteSpace(name))
        {
            if (guardian is not null)
            {
                dbContext.ClientGuardians.Remove(guardian);
                await dbContext.SaveChangesAsync();
            }
        ...
        if (guardian is null)
        {
            guardian = new ClientGuardian
            {
                TenantId = tenantContext.TenantId,
                ClientId = client.Id
            };
            dbContext.ClientGuardians.Add(guardian);
        }

        guardian.Name = name.Trim();
        guardian.Relationship = string.IsNullOrWhiteSpace(relationship) ? null : relationship.Trim();
        guardian.Email = OptionalContactFields.NormalizeEmail(email);
        guardian.Phone = string.IsNullOrWhiteSpace(phone) ? null : phone.Trim();
        guardian.Address = string.IsNullOrWhiteSpace(address) ? null : address.Trim();
        await dbContext.SaveChangesAsync();
    }
