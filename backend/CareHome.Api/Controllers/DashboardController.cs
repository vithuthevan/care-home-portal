    ...
    IOptions<CommercialRevenueFeature> commercialRevenue) : ControllerBase
{
    ...
    public async Task<ActionResult<DashboardDto>> GetDashboard()
    {
        ...
        var billingSettings = await dbContext.TenantSettings.AsNoTracking()
            .FirstAsync(x => x.TenantId == tenantContext.TenantId);
        var today = TodayInTimeZone(billingSettings.TimeZoneId);
        var upcomingRows = await dbContext.ClientFundingContracts.AsNoTracking()
            .Where(x => homes.Contains(x.Client.CareHomeId) && x.Status == "Active")
            .Select(x => new
            {
                x.Client.CareHomeId,
                CareHomePublicId = x.Client.CareHome.PublicId,
                CompanyPublicId = x.Client.CareHome.Company != null
                    ? x.Client.CareHome.Company.PublicId
                    : Guid.Empty,
                CareHomeName = x.Client.CareHome.Name,
                FundingAuthorityName = x.FundingAuthority.Name,
                x.FundingAuthority.BillingFrequency,
                x.FundingAuthority.BillingIntervalDays,
                x.FundingAuthority.CycleAnchorDate,
                x.ContractStartDate
            })
            .ToListAsync();
        var upcoming = upcomingRows
            .GroupBy(x => new { x.CareHomeId, x.FundingAuthorityName, x.BillingFrequency })
            .Select(group =>
            {
                var first = group.First();
                DateOnly? nextInvoiceDate = null;
                if (BillingPeriodModes.IsFunderCycle(billingSettings.BillingPeriodMode))
                {
                    var anchor = first.CycleAnchorDate ?? group.Min(x => x.ContractStartDate);
                    nextInvoiceDate = BillingCycleCalendar.NextCycleStart(
                        anchor,
                        today,
                        first.BillingFrequency,
                        first.BillingIntervalDays);
                }

                return new UpcomingInvoiceDto
                {
                    CareHomeId = first.CareHomeId,
                    CareHomePublicId = first.CareHomePublicId,
                    CompanyPublicId = first.CompanyPublicId,
                    ...
                    BillingPeriodMode = billingSettings.BillingPeriodMode
                };
            })
            .Take(12)
            .ToList();
    ...
    private async Task<List<string>> BuildSetupHints(int tenantId)
    {
        ...
        {
            hints.Add("Add the first resident.");
        }
